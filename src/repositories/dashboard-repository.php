...
class dashboard_repository extends base_repository
{
  ...
  {
    $query = "SELECT COUNT(*) as total FROM bookings WHERE booking_status IN (1, 2, 6)";
  ...
  public function getOccupancyRate()
  {
    $query = "SELECT 
                ROUND(
                  (COUNT(CASE WHEN booking_status IN (2, 6) THEN 1 END) * 100.0 / 
                  COUNT(*)), 
                2) as occupancy_rate 
              FROM bookings 
              WHERE booking_status IN (1, 2, 6)";
    $stmt = $this->db->prepare($query);
    $stmt->execute();
  ...
  public function getRevenueData()
  {
    $query = "SELECT 
                ...
                COALESCE(SUM(CASE WHEN MONTH(created_at) = MONTH(CURRENT_DATE()) THEN total_amount ELSE 0 END), 0) as monthly_revenue,
  ...
  public function getRecentBookings($limit = 5)
  {
    $query = "SELECT 
              ...
              WHERE b.booking_status IN (1, 2, 6)
