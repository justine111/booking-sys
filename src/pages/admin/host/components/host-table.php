<?php
$hosts = $hostManage->getAllHosts($searchQuery, $pageSize, $offset);
foreach ($hosts as $index => $row) :
?>
  ...
  <tr class="bg-white dark:bg-gray-800">
    ...
    <td class="px-6 py-3 text-gray-900 dark:text-gray-100">
      <div class="flex items-end justify-end">
        <button data-modal-target="host-details-modal-<?= htmlspecialchars($row['host_id']); ?>" data-modal-toggle="host-details-modal-<?= htmlspecialchars($row['host_id']); ?>" type="button" class="rounded-md text-sm px-2">
    ...
    <td colspan="4" class="px-6 py-12 text-center">
      <div class="flex flex-col items-center justify-center space-y-4">
        ...
        <div class="text-center">
          <h3 class="text-lg font-semibold text-gray-700 mb-1">No hosts found</h3>
          <p class="text-sm text-gray-500 max-w-sm">
            <?php if ($searchQuery): ?>
              No hosts match your search criteria "<?= htmlspecialchars($searchQuery) ?>"
            <?php else: ?>
              No hosts found in the system. Start by adding your first host.
            <?php endif; ?>
          </p>
        </div>
        <button
          data-modal-target="add-new-host"
          data-modal-toggle="add-new-host"
