<div id="booking-details-modal-<?= htmlspecialchars($row['booking_id']); ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
  <div class="relative p-4 w-full max-w-xl max-h-full">
    <!-- Modal content -->
    <div class="relative bg-white rounded-xl shadow dark:bg-gray-700">
      <div class="flex items-center justify-between p-4">
        <div>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Update Reservation</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">Fill out all required details to update the reservation</p>
        </div>
        <button type="button" class="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center" data-modal-hide="booking-details-modal-<?= htmlspecialchars($row['booking_id']); ?>">
      ...
      <form action="#" method="post" id="update-booking-form-<?= htmlspecialchars($row['booking_id']); ?>" class="update-booking-form">
        ...
        <div class="grid gap-4 grid-cols-2 py-4 p-4">
          ...
          <div class="col-span-2">
            <label for="emailAcc" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email Account <span class="text-gray-600 text-xs">(required)</span></label>
            <input
              type="email"
              name="emailAcc"
              id="emailAcc"
