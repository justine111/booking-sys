...
function getUserLocation() {
  if (!navigator.geolocation) {
    showBotMessage('Geolocation is not supported by your browser. Recommendations might not be location-specific.', 'error');
    return;
  }
...
function sendMessage() {
  ...
  fetch("/booking-sys/src/pages/ai/backend/chatbot.php", {
    ...
    .then(data => {
      ...
      if (data.error) {
        showBotMessage(data.error, 'error');
      } else {
        showBotMessage(data.response || 'No response from AI.');
      }
      userInputElement.value = '';
    })
    .catch(err => {
      loadingMessage.remove();
      showBotMessage('Failed to fetch response. Please check your network connection.', 'error');
      console.error(err);
    });
}
