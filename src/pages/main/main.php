...
// Get count and hotels based on category filter
if ($categoryFilter) {
  $count = $roomController->countHotelsByCategory($categoryFilter, $searchQuery);
  $rooms = $roomController->getHotelsByCategory($categoryFilter, $searchQuery, $pageSize, $offset);
} else {
...
$categories = $roomController->getAllCategories();
...
<section class="py-10 mt-16 mb-16">
  <div class="mx-auto max-w-screen-2xl px-4 2xl:px-0">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-10 mb-8 gap-4">
      ...
      <ul class="flex flex-wrap gap-2 text-sm font-medium" id="category-tabs" role="tablist">
        <li role="presentation">
          <button
            class="category-tab px-5 py-2 rounded-full transition-all duration-200 <?= !$categoryFilter ? 'bg-orange-500 text-white' : 'border border-gray-300 hover:bg-gray-100 text-gray-700' ?>"
            data-category=""
            type="button"
            role="tab">
            All Hotels
        ...
        <?php foreach ($categories as $category): ?>
          <li role="presentation">
            <button
              class="category-tab px-5 py-2 rounded-full transition-all duration-200 <?= $categoryFilter == $category['category_id'] ? 'bg-orange-500 text-white' : 'border border-gray-300 hover:bg-gray-100 text-gray-700' ?>"
    ...
    <!-- Hotels Grid -->
    <div id="hotels-container" class="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-6">
      <?php
    ...
    <div class="flex flex-col items-center mt-12 text-center">
      ...
      <div class="flex mt-3">
        <a href="<?= $basePath ?>/main.php?page=<?= max(1, $page - 1) ?><?= $searchQuery ? '&search=' . urlencode($searchQuery) : '' ?><?= $categoryFilter ? '&category=' . urlencode($categoryFilter) : '' ?>"
        ...
        <a href="<?= $basePath ?>/main.php?page=<?= min($totalPages, $page + 1) ?><?= $searchQuery ? '&search=' . urlencode($searchQuery) : '' ?><?= $categoryFilter ? '&category=' . urlencode($categoryFilter) : '' ?>"
...
<script>
  ...
  // Category tab switching with AJAX
  document.querySelectorAll('.category-tab').forEach(tab => {
    tab.addEventListener('click', function() {
      ...
      // Reload page to fetch new hotels
